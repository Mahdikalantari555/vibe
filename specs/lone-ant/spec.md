...
- G3 (optional for V1): If included, progress persists on the device and closing/reopening resumes exactly. May be omitted in V1 for a faster first playable.
...
- ✅ Always: keep the player controllable as exactly one ant; steer via tap-to-move (tap target, ant pathfinds); keep all food/storage logic in shared colony state; save before unload only if save is included.
...
- AC-6 (optional — only if save is included): Given the player closes the game, When it reopens, Then colony size, stored food, and unlocks match the pre-close state exactly (save-and-load).
...
Status: **in-review**. Most open questions are resolved; remaining: OQ-5
(offline save optional for V1) and OQ-6 (what "more world design" means). Do
not start implementation until the human approves this overview and the linked
feature specs. When a decision changes, update the relevant section *first*,
then implement.
