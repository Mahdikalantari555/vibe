...
- FR-2: The player SHALL be able to steer the ant's movement in the 2D plane.
- FR-3: Controls SHALL work on a touchscreen phone (no physical keyboard required).
...
- AC-2 (FR-3): Given a touchscreen device, When the player drags/taps, Then the ant moves toward the input with no keyboard.
...
- ⚠️ Ask first: changing control scheme to anything other than direct steer.
...
- EC-2: Two simultaneous touch points → use one (primary), ignore the rest.
